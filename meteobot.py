...
def set_time_of_every_day_mailing(message):
    ...
    if message.from_user.id in users:
        bot.send_message(
            message.from_user.id,
            "Choose the suitable time when I can send you daily forecast \n Format: hh:mm",
        )
...
def set_city_of_every_day_mailing(message):
    ...
    if message.from_user.id in users:
        bot.send_message(
            message.from_user.id,
            "Choose the city which weather you are interested in every day",
        )
